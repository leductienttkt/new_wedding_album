...
function zoomThis(pic) {
	...
	var	position, translate,
		...
		tmpPic.load(function() {
			var rw = $(this)[0].width, rh = $(this)[0].height, ww = $(window).width(), wh = $(window).height(),
				wscale = rw/ww, hscale = rh/wh; tl = rw/rh;
			...
			if (wscale >= 1 || hscale >= 1)
				if (wscale > hscale) {
					realWidth = ww * 0.95
					realHeight = realWidth / tl
				} else {
					realHeight = wh * 0.95
					realWidth = realHeight * tl
				}
			...
			tmpPic.attr('width', realWidth);
			tmpPic.attr('height', realHeight);
		});
...
$(document).on('ready', function() {
	...
	$('.audio-btn').on('click', function() {
		...
		if (audio.prop("paused")) {
			audio.trigger("play");
			$(this).attr("src", 'icons/pause-3.png');
		} else {
			audio.trigger("pause");
			$(this).attr("src", 'icons/play-3.png');
		}
	});
})
